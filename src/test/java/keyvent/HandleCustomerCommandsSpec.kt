...
class HandleCustomerCommandsSpec : Spek() {
    init {
        given("An empty Customer snapshot and a create command") {
            ...
            val uow: CustomerUnitOfWork? = handleCustomerCommands.invoke(state, version, cmd, stateTransitionFn)
        ...
        given("A fresh non active Customer snapshot with version 1 and an activate command") {
            ...
            val state = Customer(customerId = CustomerId(), name="customer1", active = false, activatedSince = localDatTime)
            val version = Version(1)
            val cmd = ActivateCustomerCmd(CommandId(), customerId = state.customerId!!, date = localDatTime)
            val uow: CustomerUnitOfWork? = handleCustomerCommands.invoke(state, version, cmd, stateTransitionFn)
            assertEquals(uow?.customerCommand, cmd)
